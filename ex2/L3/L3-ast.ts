...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | ClassExp;//added
...
export const makeClassExp = (fields: VarDecl[], methods: Binding[]): ClassExp =>//added
...
export const parseL3SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op ==="class" ? 
    isNonEmptyList<Sexp>(params) ? parseClassExp(first(params), rest(params)) : //added
...
const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> =>
    ...
    /*const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> =>
        isArray(fields) && allT(isString, fields) && fields.length !== 0 ?
            bind(
                mapv(
                    mapResult(parseL3CExp, methods.map(method => second(method as [Sexp, Sexp]))),
                    (vals: CExp[]) => 
                        zipWith(
                            (name, value) => makeBinding(name as string, value),
                            methods.map(method => first(method as [Sexp, Sexp])),
                            vals
                        )
                ),
                (bindings: Binding[]) => 
                    makeOk(makeClassExp(fields.map(makeVarDecl), bindings))
            ) : 
...
const unparseClassExp = (ce: ClassExp): string => 
    `(class (${ce.fields.map((field: VarDecl) => field.var).join(" ")}) ${unparseLExps(ce.methods.map((binding: Binding) => binding.val))})`
...
export const unparseL3 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp) ? unparseClassExp(exp) ://added
